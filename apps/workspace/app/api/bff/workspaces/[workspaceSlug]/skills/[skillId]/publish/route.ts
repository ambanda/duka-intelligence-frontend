import type { WorkspaceSkillPublishInput } from "@duka/api-client";
...
export async function POST(request: NextRequest, { params }: { params: Promise<{ workspaceSlug: string; skillId: string }> }) {
  try {
    ...
    const payload = await readBffJson<WorkspaceSkillPublishInput>(request);
    return NextResponse.json(sanitizeSkillDetail(await client.publishSkill(membership.workspaceId, skillId, payload)));
